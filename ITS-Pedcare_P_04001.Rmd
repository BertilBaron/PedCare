...
# compute rates
kv_data$rate <- with(kv_data, P_04001/Pop2019*10000)
...
```{r}
summary(kv_data$P_04001[kv_data$Intervention==0])
summary(kv_data$P_04001[kv_data$Intervention==1])
...
model_u_ges <- glm(P_04001 ~ offset(log(Pop2019)) + Intervention +
...
Overdispersion: Varianz größer als erwartet --> Poisson Regression nicht anwendbar
```{r Poisson regression model overdispersion}
...
model2_u_ges <- glm(P_04001 ~ offset(log(Pop2019)) + Intervention +
...
model3_u_ges <- glm.nb(P_04001 ~ offset(log(Pop2019)) + Intervention +
                         Zeit + Intervention*Zeit,
                       data = kv_data)
...
```{r}
model4_u_ges <- glm.nb(P_04001 ~ offset(log(Pop2019)) + Intervention +
                         Zeit + Intervention*Zeit +
                         Ferien + Feiertag,
                       data = kv_data)
...
```{r}
model6_u_ges <- glm.nb(P_04001 ~ offset(log(Pop2019)) + Intervention +
                         Zeit + Intervention*Zeit +
...
### Frage: Reichen uns die Daten aus, um die typische Saisonalität zu modellieren oder wird es nicht zum Teil durch den Pandemie-Effekt modelliert?
...
```{r}
model_u_ges_test <- glm(P_04001 ~ offset(log(Pop2019)) + Intervention +
                      Zeit + Intervention*Zeit +
                      harmonic(Monat,2,12),
                    family=quasipoisson, kv_data)
...
### Löschen der Wochen im März 2020 vor dem, da unsicher, inwiefern die Intervention da schon gewirkt hat
```{r}
kv_data1 <- kv_data %>%
...
kv_data3 <- kv_data %>%
  mutate(Intervention2 = case_when(Zeit < 65 ~ "phase1",
                                   ...
                                   Zeit >= 79 ~ "phase3"))
model8_u_ges <- glm.nb(P_04001 ~ offset(log(Pop2019)) + Intervention2 +
